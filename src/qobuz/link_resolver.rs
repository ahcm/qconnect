...
mod tests
{
    ...
    fn test_random_text()
    {
        assert_eq!(resolve_link("not a url at all"), Err(LinkResolverError::UnsupportedScheme));
    }
    ...
    {
        assert_eq!(resolve_link("qobuzapp://"), Err(LinkResolverError::MalformedUrl));
    }

    #[test]
    fn test_scheme_only_entity_no_id()
    {
        assert_eq!(resolve_link("qobuzapp://album"), Err(LinkResolverError::MalformedUrl));
    }
}

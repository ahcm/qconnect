...
use crate::transport::{QueueItem, QueueVersion};
use crate::transport::{TransportEvent, WsTransport, WsTransportConfig};
    ...
    S: QconnectEventSink + 'static,
{
    ...
    {
        let envelope =
            build_qconnect_renderer_outbound_envelope(report).map_err(|e| anyhow!("{}", e))?;
        self.transport
            .send(envelope)
            .await
            .map_err(|e| anyhow!("{}", e))
    ...
    pub async fn handle_transport_event(&self, event: TransportEvent) -> Result<()>
    {
        match event
        {
            TransportEvent::InboundRendererServerCommand(cmd) =>
            {
                ...
                self.sink
                    .on_event(QconnectAppEvent::RendererCommandApplied { command })
                    .await;
            }
            TransportEvent::InboundQueueServerEvent(evt) => match evt.event_type
            {
                QueueEventType::SrvrCtrlQueueState =>
                {
                    ...
                    .unwrap_or_default();
                    let shuffle_mode = evt
                        .payload
                        .get("shuffle_mode")
                        .and_then(Value::as_bool)
                        .unwrap_or(false);
                    let autoplay_mode = evt
                        .payload
                        .get("autoplay_mode")
                        .and_then(Value::as_bool)
                        .unwrap_or(false);
                    ...
                    *self.queue_state.lock().await = new_queue.clone();
                    self.sink
                        .on_event(QconnectAppEvent::QueueUpdated(new_queue))
                        .await;
                }
                ...
                QueueEventType::SrvrCtrlQueueTracksRemoved =>
                {
                    ...
                    queue
                        .queue_items
                        .retain(|it| !remove_ids.contains(&it.queue_item_id));
                    let new_queue = queue.clone();
                    drop(queue);
                    self.sink
                        .on_event(QconnectAppEvent::QueueUpdated(new_queue))
                ...
                QueueEventType::SrvrCtrlRendererStateUpdated =>
                {
                    ...
                    let current_queue_item_id = player_state
                        .and_then(|s| s.get("current_queue_item_id"))
                        .and_then(|v| v.as_u64());
                    let next_queue_item_id = player_state
                        .and_then(|s| s.get("next_queue_item_id"))
                        .and_then(|v| v.as_u64());
                    let current_position_ms = player_state
                        .and_then(|s| s.get("current_position"))
                        .and_then(|v| v.as_u64());
                    let playing_state = player_state
                        .and_then(|s| s.get("playing_state"))
                        .and_then(|v| v.as_i64())
                        .map(|v| v as i32);

                    log::debug!(
                        "RENDERER_STATE_UPDATED: current_id={:?}, next_id={:?}",
                    ...
                    {
                        log::warn!(
                            "Could not find current_track for queue_item_id {:?}",
                            current_queue_item_id
                        );
                    }
                    ...
                    *self.renderer_state.lock().await = new_renderer.clone();
                    self.sink
                        .on_event(QconnectAppEvent::RendererUpdated(new_renderer))
                        .await;
                }
